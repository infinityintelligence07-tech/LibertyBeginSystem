...
BEGIN
  ...
  IF COALESCE(v_is_kickoff, false) = false
     AND NEW.status::text NOT IN ('cancelled', 'not_realized') THEN
    SELECT count(*)
...
REVOKE ALL ON FUNCTION public.enforce_member_booking_rules() FROM PUBLIC;

DROP TRIGGER IF EXISTS trg_enforce_member_booking_rules ON public.bookings;
CREATE TRIGGER trg_enforce_member_booking_rules
BEFORE INSERT OR UPDATE OF status, scheduled_date, liberty_id, session_id
ON public.bookings
FOR EACH ROW
EXECUTE FUNCTION public.enforce_member_booking_rules();