...
AS $$
DECLARE
  v_kickoff boolean := false;
BEGIN
  SELECT COALESCE(is_kickoff, false)
    INTO v_kickoff
  FROM public.sessions
  WHERE id = NEW.session_id;

  NEW.report_required := NOT (v_kickoff OR COALESCE(NEW.is_retroactive, false));
  RETURN NEW;
END;
$$;
...
BEFORE INSERT OR UPDATE OF session_id, is_retroactive ON public.bookings
...
BEGIN
  ...
  SELECT COALESCE(is_kickoff, false), name
    INTO v_is_kickoff, v_session_name
  FROM public.sessions
  WHERE id = NEW.session_id;
  IF v_is_kickoff THEN RETURN NEW; END IF;
...
BEGIN
  FOR m IN
    ...
    WHERE bk.status IN ('scheduled','rescheduled','completed')
      ...
      AND COALESCE(s.is_kickoff, false) = false
  ...
  LOOP
    ...
    INSERT INTO public.notifications(user_id, type, title, message, link, related_booking_id)
    VALUES (
      v_mentor_user, 'report_overdue',
      CASE WHEN m.pending_count > 1 THEN '⚠️ ' || m.pending_count || ' relatórios pendentes'
           ELSE '⚠️ Relatório pendente — sessão não contabilizada' END,
      CASE WHEN m.pending_count > 1
        THEN 'Você tem ' || m.pending_count || ' sessões sem relatório (a mais antiga: ' || v_first_label || '). Sem o relatório, elas não contam como realizadas.'
        ELSE v_first_label || ' ainda não tem relatório. Sem ele, a sessão não conta como realizada.' END,
      '/mentor/sessoes/' || v_first_booking::text || '/relatorio', v_first_booking
    );
  END LOOP;
END;
$function$;
...
REVOKE ALL ON FUNCTION public.dispatch_report_nudges() FROM PUBLIC, anon, authenticated;